...
document.writeln('<iframe id=endDateLayer src="1.html" frameborder=0 width=162 height=211 style="position: absolute;  z-index: 9998; display: none"></iframe>');
strFrame='<style>';
strFrame+='INPUT.button{BORDER-RIGHT: #336699 1px solid;BORDER-TOP: #336699 1px solid;BORDER-LEFT: #336699 1px solid;';
strFrame+='BORDER-BOTTOM: #336699 1px solid;BACKGROUND-COLOR: #336699;font-family:宋体;}';
...
strFrame+='		DateLayer.posLeft += window.event.clientX-datelayerx+100;';
strFrame+='		DateLayer.posTop += window.event.clientY-datelayery+100;}}';
strFrame+='function DragStart()';
strFrame+='{var DateLayer=parent.document.all.endDateLayer.style;';
strFrame+='	datelayerx=window.event.clientX+100;';
strFrame+='	datelayery=window.event.clientY+100;';
strFrame+='	bDrag=true;}';
...
strFrame+='<table border=1 cellspacing=0 cellpadding=0 width=142 height=160 bordercolor=#336699 bgcolor=#336699 >';
...
strFrame+='                <td width=16 align=center bgcolor=#336699 style="font-size:12px;cursor: hand;color: #ffffff" ';
strFrame+='					   onclick="parent.meizzPrevM()" title="向前翻 1 月" ><b >&lt;</b></td>';
strFrame+='				   <td width=60 align="center" bgcolor="#336699"  style="font-size:12px;cursor:hand" ';
strFrame+='				       onmouseover="style.backgroundColor=\'#aaccf3\'"';
strFrame+='					   onmouseout="style.backgroundColor=\'#336699\'" ';
...
strFrame+='					   bgcolor="#336699" onmouseover="style.backgroundColor=\'#aaccf3\'" ';
strFrame+='					   onmouseout="style.backgroundColor=\'#336699\'" ';
...
strFrame+='				   <td width=16 bgcolor=#336699 align=center style="font-size:12px;cursor: hand;color: #ffffff" ';
strFrame+='					   onclick="parent.meizzNextM()" title="向后翻 1 月" ><b >&gt;</b></td>';
strFrame+='			   </tr>';
strFrame+='		   </table></td></tr>';
strFrame+='	   <tr><td width=142 height=18 >';
strFrame+='		   <table border=0 cellspacing=0 cellpadding=2 bgcolor=#336699 ' + (bMoveable? 'onmousedown="DragStart()" onmouseup="DragEnd()"':'');
strFrame+='				BORDERCOLORLIGHT=#336699 BORDERCOLORDARK=#FFFFFF width=140 height=20  style="cursor:' + (bMoveable ? 'move':'default') + '">';
...
strFrame+='    <table border=1 cellspacing=2 cellpadding=2 BORDERCOLORLIGHT=#336699 BORDERCOLORDARK=#FFFFFF bgcolor=#fff8ec width=140 height=120 >';
...
strFrame+='          <tr bgcolor="#336699"><td id=bUseTimeLayer width=30  style="cursor:hand" title="点击这里启用/禁用时间"';
strFrame+='				onmouseover="style.backgroundColor=\'#aaccf3\'" align=center onmouseout="style.backgroundColor=\'#336699\'"';
...
strFrame+=' onmouseover="style.backgroundColor=\'#aaccf3\'" onmouseout="style.backgroundColor=\'#336699\'"';
...
function setday(tt,obj) //主调函数
{
	...
	var tleft = tt.offsetLeft-150;	//TT控件的定位点宽
...
function meizzSetDay(yy,mm)	//主要的写程序**********
{
	...
	{
		//初始化边框
		da.borderColorLight="#336699";
		da.borderColorDark="#336699";
...
/**
 * n个月后;要求n大于1小于12,格式为y-m-d,
 * @author xuhj
 */
function getMonthLater(date,index)
{

	if(date && index && date != "" && index != "")
	{
		var begin = Date.parseDate(date, "Y-m-d");
		var month = begin.getMonth() + 1;//月份
		var year = begin.getYear();//年份
		var day = begin.getDate();//日
		var returnDate = "";
		
		if(month >=1 && month <=12-index)
		{
			month = month + index;
		}else
		{
		...
		//如果是闰年，如果算出来为2月30号， 2月31号，转换成2月29号，非闰年转换成2月28号
		//其他月份如果不是闰年，如果是4，6，9，11月，且月31号，转换成30号
		if(year % 4 == 0)
		{
